...
function profileInfoSection(user, profile) {
  return `
    <div class="card border-0 shadow-sm">
      <div class="card-body">
        <h2 class="h5 mb-3">Информация за профил</h2>
        <div class="row g-3">
          <div class="col-12 col-md-4">
            <div class="small text-muted">Пълно име</div>
          ...
          <div class="card-body">
            ...
            ${
              trip.moderation_status === "pending"
                ? '<div class="alert alert-warning py-2 mb-3">Това пътуване очаква одобрение на администратор.</div>'
                : ""
            }
            ${
...
function adminPendingTripsSection(trips) {
  if (!trips.length) {
    return '<div class="alert alert-secondary mb-0">Няма очаквани пътувания, които очакват одобрение.</div>';
  }
...
async function renderProfileContent(user, profile) {
  ...
  if (adminTripsHost) {
    adminTripsHost.innerHTML = '<div class="text-muted">Зареждане на очаквани пътувания...</div>';
  }
...
function bindDriverActions(user, profile) {
  ...
  driverHost.addEventListener("click", async (event) => {
    ...
    if (driverId !== user.id) {
      showInlineAlert("driverTripsContent", "Не е разрешено да управлявате тази резервация.");
      return;
    }
    ...
    try {
      ...
      showToast(`Резервацията ${action} успешно.`);
...
function bindAdminTripModerationActions(user, profile) {
  ...
  adminTripsHost.addEventListener("click", async (event) => {
    ...
    try {
      ...
      showToast(`Пътуването ${action} успешно.`);
      await renderProfileContent(user, profile);
    } catch (error) {
      showInlineAlert("adminTripModerationContent", error.message || "Невъзможно модериране на пътуванието.");
    ...
    <main class="container py-5">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p class="text-muted mb-0">Управлявайте вашия акаунт и заявките за резервация.</p>
        </div>
      </div>

      <ul class="nav nav-tabs mb-3" id="profileTabs" role="tablist">
        <li class="nav-item" role="presentation">
          <button class="nav-link active" id="info-tab" data-bs-toggle="tab" data-bs-target="#info-pane" type="button" role="tab" aria-controls="info-pane" aria-selected="true">Информация за профил</button>
        </li>
        <li class="nav-item" role="presentation">
          <button class="nav-link" id="bookings-tab" data-bs-toggle="tab" data-bs-target="#bookings-pane" type="button" role="tab" aria-controls="bookings-pane" aria-selected="false">Моите резервации</button>
        </li>
        <li class="nav-item" role="presentation">
      ...
      <section id="adminTripModerationSection" class="card border-0 shadow-sm mt-4 d-none">
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <span class="small text-muted">Администраторски модерационен ред</span>
          </div>
